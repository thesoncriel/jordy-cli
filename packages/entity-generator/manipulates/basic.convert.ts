import { hasPathParameter, snakeToCamel, toCapitalize } from '../../common';
import {
...
export function extractBaseEntityNameFromPath(
  ...
  let tmpPath =
    Array.isArray(removeKeywords) && removeKeywords.length > 0
      ? path.replace(new RegExp(removeKeywords.join('|'), 'g'), '')
      : path;
